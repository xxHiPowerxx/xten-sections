...
function component_hero( $args = null ) {
	...
	foreach( $slides as $key => $slide ) :
		...
		// Minimum Height
		$minimum_height       = esc_attr( $slide['minimum_height'] );
		$sizeHero             = null;
		if ( $minimum_height ) :
			$minimum_height_unit     = esc_attr( $slide['minimum_height_unit'] ) ? : '%'; // DV = '%'
			$minimum_height_attr_val = $minimum_height . $minimum_height_unit;
			$slide_attrs['data-minimum-height'] = $minimum_height_attr_val;
			if ( $minimum_height_unit === '%' ) :
				$slide_attrs['class'] .= ' sizeHeroParent';
				$sizeHero = 'sizeHero';
			else :
				$styles .= xten_add_inline_style(
					$slide_selector . ' .container-xten-component-hero',
					array(
